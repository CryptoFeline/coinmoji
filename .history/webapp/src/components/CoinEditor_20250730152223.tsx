    ...
    const animate = () => {
      ...
      const updateAnimatedTexture = (material: THREE.MeshStandardMaterial) => {
        if (material.map) {
          ...
          } else if (material.map instanceof THREE.CanvasTexture) {
            // Update GIF canvas textures
            const img = (material.map as any)._animatedImage;
            const canvas = (material.map as any)._canvas;
            const ctx = (material.map as any)._context;
