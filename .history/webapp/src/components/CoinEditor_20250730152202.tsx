...
const CoinEditor: React.FC<CoinEditorProps> = ({ className = '', settings: externalSettings }) => {
  ...
  // Helper to detect if URL is animated
  ...
  const createTextureFromUrl = (url: string): Promise<THREE.Texture | THREE.VideoTexture | THREE.CanvasTexture> => {
    return new Promise((resolve, reject) => {
      ...
      if (isGif) {
        // Special handling for GIFs using canvas texture for animation
        const img = new Image();
        ...
        img.onload = () => {
          ...
          // Draw the image to canvas (this will animate automatically for GIFs)
          ...
          (canvasTexture as any)._animatedImage = img;
          (canvasTexture as any)._canvas = canvas;
          (canvasTexture as any)._context = ctx;
          
          if (sceneRef.current) {
            canvasTexture.anisotropy = sceneRef.current.renderer.capabilities.getMaxAnisotropy();
          }
