...
const CoinEditor: React.FC<CoinEditorProps> = ({ className = '', settings: externalSettings }) => {
  ...
  const applyOverlay = (url: string, isSecond = false, requestId = overlayReqIdRef.current) => {
    ...
    createTextureFromUrl(url)
      .then((texture) => {
        ...
        // Double-check scene is still available
        if (!sceneRef.current) {
          (texture as any).userData?.dispose?.();
          texture.dispose?.();
          return;
        ...
        if (currentSettings.dualOverlay && isSecond) {
          ...
          // Apply to both faces - first image in dual overlay mode (acts as fallback for both faces)
          ...
          // Apply to bottom face with proper orientation
          if (texture instanceof THREE.VideoTexture) {
            const video = (texture as any).image;
  ...
  const clearOverlays = (invalidate: boolean = true) => {
    ...
    // Force a render to ensure changes are applied immediately
  ...
  useEffect(() => {
    ...
    // Apply overlays for this request - handle both single and dual overlay modes
    const hasFirstOverlay = currentSettings.overlayUrl && currentSettings.overlayUrl.trim() !== '';
    const hasSecondOverlay = currentSettings.dualOverlay && currentSettings.overlayUrl2 && currentSettings.overlayUrl2.trim() !== '';
    
    // In dual overlay mode, apply first image to both faces first (as fallback)
    if (hasFirstOverlay) {
      applyOverlay(currentSettings.overlayUrl, false, reqId);
    }
    
    // Then apply second image to bottom face only (if exists, will override first image on bottom)
    if (hasSecondOverlay) {
      applyOverlay(currentSettings.overlayUrl2, true, reqId);
    }
  }, [currentSettings.overlayUrl, currentSettings.overlayUrl2, currentSettings.dualOverlay, currentSettings.gifAnimationSpeed]);
