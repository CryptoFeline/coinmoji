...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ isOpen, onClose, settings, onSettingsChange }) => {
  ...
  const handleClearOverlay = () => {
    ...
    // Clear temp fields first
    setTempOverlayUrl('');
    setTempOverlayUrl2('');
    
    // Then clear the actual settings - use setTimeout to ensure state updates are processed
    setTimeout(() => {
      updateSetting('overlayUrl', '');
      updateSetting('overlayUrl2', '');
    }, 0);
    
    console.log('Clear button clicked - after clear calls made');
  };
