...
const CoinEditor: React.FC<CoinEditorProps> = ({ className = '', settings: externalSettings }) => {
  ...
  useEffect(() => {
    ...
    const shouldClear = !currentSettings.overlayUrl && !currentSettings.overlayUrl2;
    
    if (shouldClear) {
      console.log('Clearing all overlays');
      clearOverlays();
      return; // Exit early, don't apply anything
    }
    
    // Clear overlays before applying new ones (but not if we just cleared above)
    clearOverlays();
