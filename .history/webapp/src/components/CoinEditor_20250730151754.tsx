...
const CoinEditor: React.FC<CoinEditorProps> = ({ className = '', settings: externalSettings }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Update video textures for animation
      if (sceneRef.current.overlayTop.material) {
        ...
        if (topMaterial.map && topMaterial.map instanceof THREE.VideoTexture) {
      ...
      if (sceneRef.current.overlayBot.material) {
        ...
        if (botMaterial.map && botMaterial.map instanceof THREE.VideoTexture) {
          botMaterial.map.needsUpdate = true;
        }
      }
      
      // Update video textures on coin faces and rim
      sceneRef.current.coinGroup.children.forEach(child => {
        if (child instanceof THREE.Mesh && child.material) {
          const materials = Array.isArray(child.material) ? child.material : [child.material];
          materials.forEach(material => {
            if (material instanceof THREE.MeshStandardMaterial && material.map && material.map instanceof THREE.VideoTexture) {
  ...
  // Helper to detect if URL is animated WebM (GIFs will be handled as regular images since they animate automatically)
  ...
  const createTextureFromUrl = (url: string): Promise<THREE.Texture | THREE.VideoTexture> => {
    return new Promise((resolve, reject) => {
      if (isAnimatedUrl(url)) {
        // Create video element for WebM content only
        ...
        const handleVideoSuccess = () => {
          try {
            ...
            videoTexture.flipY = false; // Important for proper orientation
        ...
        const fallbackToImage = () => {
          ...
          loader.load(
            ...
            (texture) => {
              ...
              texture.flipY = false; // Consistent with video texture
              if (sceneRef.current) {
        ...
        // Use standard texture loader for static images and GIFs (GIFs animate automatically in the browser)
        const loader = new THREE.TextureLoader();
        loader.setCrossOrigin('anonymous');
        loader.load(
          url,
          (texture) => {
            texture.colorSpace = THREE.SRGBColorSpace;
            texture.flipY = false; // Consistent orientation
            if (sceneRef.current) {
              texture.anisotropy = sceneRef.current.renderer.capabilities.getMaxAnisotropy();
            }
            resolve(texture);
    ...
    createTextureFromUrl(url)
      .then((texture) => {
        ...
        if (currentSettings.dualOverlay && isSecond) {
          ...
          // Clone the texture for bottom to avoid sharing issues
  ...
  const clearOverlays = () => {
    ...
    if (topMaterial.map) {
      console.log('Disposing top material map');
      topMaterial.map.dispose();
      topMaterial.map = null;
    }
    if (botMaterial.map) {
      console.log('Disposing bot material map');
      botMaterial.map.dispose();
      botMaterial.map = null;
    }
    
    // Reset material properties and make overlays invisible
    ...
    console.log('Overlays cleared, visibility set to false');
  ...
  useEffect(() => {
    ...
    console.log('Overlay effect triggered:', { 
      overlayUrl: currentSettings.overlayUrl, 
      overlayUrl2: currentSettings.overlayUrl2, 
      dualOverlay: currentSettings.dualOverlay 
    });
    ...
    if (currentSettings.overlayUrl) {
      console.log('Applying first overlay:', currentSettings.overlayUrl);
      applyOverlay(currentSettings.overlayUrl, false);
    }
    
    if (currentSettings.dualOverlay && currentSettings.overlayUrl2) {
      console.log('Applying second overlay:', currentSettings.overlayUrl2);
      applyOverlay(currentSettings.overlayUrl2, true);
    }
    
    // Force update if both URLs are empty to ensure clearing works
    if (!currentSettings.overlayUrl && !currentSettings.overlayUrl2) {
      console.log('Both overlay URLs empty, forcing clear');
      setTimeout(() => clearOverlays(), 50); // Slight delay to ensure state is stable
    }
