...
const CoinEditor: React.FC<CoinEditorProps> = ({ className = '', settings: externalSettings }) => {
  ...
  const gifUrlToVideoTexture = async (url: string): Promise<THREE.VideoTexture> => {
    // 1) Fetch GIF bytes
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      delays.push(Math.max(2, d) * 10); // → ms
      if (f.disposalType === 2) {
    ...
    for (let i = 0; i < fullFrames.length; i++) {
      ...
      // VideoFrame from ImageData with alpha
      const frame = new (window as any).VideoFrame(id, { timestamp: Math.round(ts * 1000) });
  ...
  const createTextureFromUrl = (url: string): Promise<THREE.Texture | THREE.VideoTexture | THREE.CanvasTexture> => {
    return new Promise((resolve, reject) => {
      ...
      if (isGif) {
        // Try treating GIF as WebM first (some servers convert automatically)
        const tryAsWebM = () => {
          const video = document.createElement('video');
          ...
          const handleVideoSuccess = () => {
            try {
              ...
              videoTexture.flipY = true; // Video textures need flipY=true
              resolve(videoTexture);
            } catch (error) {
              console.warn('GIF as WebM failed, trying canvas approach:', error);
              tryAsCanvas();
            }
          };
          
          const tryAsCanvas = () => {
            // Animated GIF → drive a CanvasTexture from a hidden <img>
            const img = new Image();
            img.crossOrigin = 'anonymous';
            // Keep GIF animating by attaching it invisibly to the DOM
            ...
            img.onload = () => {
              ...
              tex.colorSpace = THREE.SRGBColorSpace;
              tex.flipY = true; // Match video texture orientation
              if (sceneRef.current) {
                tex.anisotropy = sceneRef.current.renderer.capabilities.getMaxAnisotropy();
              }

              // Per‑frame updater (called from main rAF loop)
              tex.userData.update = () => {
          ...
          video.addEventListener('canplay', () => {
            video.play().then(handleVideoSuccess).catch(tryAsCanvas);
          });
          ...
          video.addEventListener('error', tryAsCanvas);
          video.src = url;
          ...
          }, 1000); // Shorter timeout for GIF fallback
        };
    ...
    createTextureFromUrl(url)
      .then((texture) => {
        ...
        if (currentSettings.dualOverlay && isSecond) {
          ...
          // Bottom face uses a clone so we can manage independently
  ...
  const clearOverlays = (invalidate: boolean = true) => {
    ...
    // IMPORTANT: Force re-application of body materials to show underlying coin appearance
    // This ensures the coin face shows its proper color/texture/gradient when overlays are cleared
    const { rimMat, faceMat } = sceneRef.current;
    
    if (currentSettings.bodyTextureUrl) {
      // Reapply body texture
      createTextureFromUrl(currentSettings.bodyTextureUrl)
        .then((texture) => {
          // Apply planar UV mapping for face textures
          const { coinGroup } = sceneRef.current!;
          const faces = coinGroup.children.filter(child => 
            child instanceof THREE.Mesh && 
            (child.material === faceMat || child.material.uuid === faceMat.uuid)
  ...
  useEffect(() => {
    ...
    const nothingToShow = !currentSettings.overlayUrl && !currentSettings.overlayUrl2;
    if (nothingToShow) {
